...
trait Repository {
  ...
  def updatePatient(namePatient: String, fieldToChange: String, valueToChange: String): JsObject
}


case class RepositoryImpl(databaseName: String = "Hospital") extends Repository {
  private val client = MongoClient()
  private val db = client.getDatabase(databaseName)

  override def findPatient(namePatient: String): JsObject = {
    val coll: MongoCollection[BsonDocument] = db.getCollection[BsonDocument]("Pazienti")
    coll.find(equal("name", "Enrico")).JsonResults()
  }

  override def updatePatient(namePatient: String, fieldToChange: String, valueToChange: String): JsObject = {
    val coll: MongoCollection[BsonDocument] = db.getCollection[BsonDocument]("Pazienti")
    //Set only one property
    coll.updateOne(equal("name", namePatient), set(fieldToChange, valueToChange)).JsonResults()
  }
...
object WritePatient extends App {
  ...
  val document = BsonDocument("name" -> "Café Con Leche",
  ...
  coll.updateOne(equal("name", "Enrico"), set("name", "Marco")).printHeadResult("Update Result: ")
